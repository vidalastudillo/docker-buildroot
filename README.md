...
named volume inside the VM disk. Run once per machine. See [macOS notes](#macos-notes).
...
The `buildroot_workspace` named volume is shared across all externals and holds:
...
./scripts/colima.sh status  # VM status and workspace volume info
```

`colima.sh` sets `COLIMA_HOME` automatically — no manual profile export
