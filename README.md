...
On macOS with Colima, use the provided `scripts/containers.sh` to host
the Docker VM on an external SSD. This prevents internal SSD wear from the
heavy I/O generated by Buildroot builds:

```shell
./scripts/containers.sh setup   # once — creates the VM on the external SSD
./scripts/containers.sh up      # daily startup
```
